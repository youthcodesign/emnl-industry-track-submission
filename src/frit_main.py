...
def parse_data(train_df, val_df, test_df, gold_df):
    ...
    gold_texts = test_df.TEXT.values.tolist()
    gold_labels = test_df.ROLE.values.tolist()
...
def main():
    ...
    project_name = f"FRIT_{args.model_name.split('/')[-1]}_{args.strategy}_{args.data_point}_{args.fold}_upsample={args.up_sample}"
    ...
    training_config = TrainingConfig(
        ...
        use_ewc=getattr(config, 'use_ewc', True)
    )

    # Load data
    up_train, up_val, down_train, down_val, updown_test, updown_gold = get_friten_data(args.directory, args.fold, args.strategy)
    ...
    try:
        if args.strategy == 'multiclass_scratch':
            ...
            #train_df = up_train.merge(down_train, on=['TEXT'], how='left').dropna().drop_duplicates()
            #val_df = up_val.merge(down_val, on=['TEXT'], how='left').dropna().drop_duplicates()

            # Log data balance
            #print_data_balance_report(train_df, val_df, test_df, 'HATE', gold_df)
            ...
            dir_name = 'FRIT_'+args.directory.split('/')[-1]
            csv_name = f"{project_name}_{run_name}_{dir_name}_upsample={args.up_sample}"
            ...
            mname = args.model_name.split("/")[-1]
            dir_name = 'FRIT_'+args.directory.split('/')[-1]
            csv_name = f"{project_name}_{run_name}_{dir_name}"
...
if __name__ == "__main__":
    args = get_args()
    project_name = f"FRIT_{args.model_name.split('/')[-1]}_{args.strategy}_fr-it-en_{args.fold}_upsample={args.up_sample}" 
