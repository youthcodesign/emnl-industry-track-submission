...
class TrainingConfig:
    ...
    model_name: str = "roberta-base" # or "bert-base-uncased", "facebook/opt-350m"
    max_length: int = 512
    batch_size: int = 16
...
class TransformerClassifier(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        if hasattr(outputs, 'pooler_output') and outputs.pooler_output is not None:
            ...
            if self.config.model_type == "gpt2" or "opt" in self.config.model_type:
...
class BullyingClassifierTrainer:
    ...
    def train_model(self, model: nn.Module, train_loader: DataLoader, 
        ...
        """Generic training function with wandb logging"""
    ...
    def train_multiclass_from_scratch(self, train_texts: List[str], train_labels: List[int],
                                    val_texts: List[str], val_labels: List[int]) -> Dict:
        """Strategy 1: Multiclass classifier from scratch"""
        logger.info("Training multiclass classifier from scratch...")
        ...
        results = self.train_model(model, train_loader, val_loader, model_name="multiclass_scratch")
    ...
    def train_hierarchical_pairwise_classifiers(self, 
        ...
        """Strategy: Hierarchical 2-level pairwise classification (bully vs victim -> specific roles)"""
    ...
    def predict_hierarchical_pairwise(self, models: Dict[str, nn.Module], test_texts: List[str], 
        ...
        Computes P(role|x) for all 4 roles: bully, bully_support, victim, victim_support
...
def get_en_data(directory, data_point, fold, strategy):
    ...
    if directory == 'role_off' and strategy != 'multiclass_scratch':
...
def get_friten_data(base_dir, fold, strategy):
    ...
    if strategy != 'multiclass_scratch':
